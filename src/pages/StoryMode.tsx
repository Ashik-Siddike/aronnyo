...
const KEY = 'story_progress', PEND = 'story_pending';
type LP = { stars: number; done: boolean };
type Prog = Record<number, LP>;
const load = (): Prog => { try { return JSON.parse(localStorage.getItem(KEY) || '{}'); } catch { return {}; } };
const save = (p: Prog) => localStorage.setItem(KEY, JSON.stringify(p));
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 backdrop-blur-sm px-6">
      <div className="w-full max-w-sm rounded-3xl p-8 shadow-2xl text-center"
        style={{
          background: isDark ? 'linear-gradient(145deg,#1e1b4b,#0f172a)' : 'white',
        ...
        <div className="w-20 h-20 rounded-3xl mx-auto mb-4 flex items-center justify-center text-4xl"
          style={{ background: isDark ? lvl.darkBg : lvl.bg }}>
          {lvl.emoji}
        </div>
        <h2 className="text-2xl font-black mb-1" style={{ color: isDark ? '#f1f5f9' : '#1e293b' }}>
          {t.wellDone} 🎉
        </h2>
        ...
        <div className="flex justify-center gap-4 mb-8">
          {[1, 2, 3].map(s => (
            <button key={s} onMouseEnter={() => setHov(s)} onMouseLeave={() => setHov(0)} onClick={() => setPick(s)} className="transition-all hover:scale-125 active:scale-90">
...
export default function StoryMode() {
  ...
  useEffect(() => {
    const p = load(); setProg(p);
    const raw = localStorage.getItem(PEND);
    if (raw) {
      localStorage.removeItem(PEND);
      const id = +raw; const lvl = LEVELS.find(l => l.id === id);
  ...
  const rate = (stars: number) => {
    if (!modal) return;
    const u = { ...prog, [modal.id]: { stars, done: true } };
    setProg(u); save(u); setModal(null);
  ...
  // ── Dark / Light tokens ──
  const pageBg = isDark
    ? 'linear-gradient(160deg,#0f172a 0%,#1e1b4b 50%,#0f0a1e 100%)'
    : 'linear-gradient(160deg,#f0f4ff 0%,#faf5ff 50%,#fff7ed 100%)';
  ...
  const backTextColor = isDark ? '#94a3b8' : '#64748b';
  const subTextColor = isDark ? '#64748b' : '#94a3b8';
  const progressTrackColor = isDark ? '#1e293b' : '#f1f5f9';

  const bottomPanelBg = isDark ? 'rgba(15,23,42,0.97)' : 'rgba(255,255,255,0.9)';
  const bottomBorderColor = isDark ? 'rgba(139,92,246,0.2)' : 'rgba(241,245,249,1)';
  ...
  const nodeTagLockedColor = isDark ? '#475569' : '#94a3b8';
  const nodeTagBg = (isSel: boolean, lvlColor: string) =>
    isSel ? `${lvlColor}22` : isDark ? 'rgba(15,23,42,0.75)' : 'rgba(255,255,255,0.75)';
  const nodeTagBorder = (isSel: boolean, lvlColor: string) =>
    isSel ? `${lvlColor}50` : isDark ? 'rgba(139,92,246,0.15)' : 'rgba(0,0,0,0.05)';
      ...
      <div className="shrink-0 flex items-center justify-between px-5 py-3 backdrop-blur-md shadow-sm"
        ...
        <div className="text-center">
          <span className="font-black text-base" style={{
            ...
            🗺️ {t.adventureMapTitle}
          </span>
          <p className="text-[11px] font-medium" style={{ color: subTextColor }}>{earned}/{total} ⭐</p>
        </div>
        {/* Circular progress */}
        <div className="relative w-10 h-10">
          <svg className="w-10 h-10 -rotate-90" viewBox="0 0 40 40">
            <circle cx="20" cy="20" r="16" fill="none" stroke={progressTrackColor} strokeWidth="4" />
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden" style={{ scrollbarWidth: 'none' }}>
        <div className="relative mx-auto" style={{ width: '100%', maxWidth: `${W}px`, height: `${H}px` }}>
          ...
          <svg className="absolute inset-0 w-full h-full" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
            ...
            {SEGMENTS.map((seg, i) => {
              const destLvl = LEVELS.find(l => l.id === seg.to)!;
              const open = isOpen(seg.to, prog);
              const done = !!prog[seg.to]?.done;
              const color = done ? destLvl.color : open ? destLvl.color : (isDark ? '#334155' : '#cbd5e1');
              const opacity = open ? 1 : 0.35;
              return (
                <g key={i}>
                  {open && (
                    <path d={seg.d} fill="none"
                      stroke={destLvl.color} strokeWidth="10" strokeLinecap="round"
              ...
              <button key={node.id}
                ...
                <div className="relative flex items-center justify-center rounded-full"
                  style={{
                    ...
                    background: open
                      ? isDark
                        ? `radial-gradient(circle at 36% 30%, ${lvl.darkBg} 0%, ${lvl.darkBg} 40%, ${lvl.color}40 100%)`
                        : `radial-gradient(circle at 36% 30%, white 0%, ${lvl.bg} 35%, ${lvl.color}30 100%)`
                      : nodeLocked,
                    border: `3.5px solid ${open ? lvl.color : nodeBorderLocked}`,
                    boxShadow: isSel
                      ? `0 0 0 5px ${lvl.color}30, 0 10px 28px ${lvl.color}50`
                      : done
                        ? `0 4px 16px ${lvl.color}40, 0 2px 6px rgba(0,0,0,0.1)`
                ...
                <div className="flex gap-1 mt-2">
                  {[1, 2, 3].map(s => (
                    <Star key={s} className={`w-3.5 h-3.5 transition-all ${s <= (lp?.stars ?? 0) ? 'fill-amber-400 text-amber-400' : isDark ? 'fill-slate-700 text-slate-700' : 'fill-slate-200 text-slate-200'}`}
                ...
                <div className="mt-1.5 px-3 py-1 rounded-xl text-center font-black"
                  style={{
                    fontSize: 9.5, maxWidth: 90, lineHeight: 1.3,
                    color: open ? lvl.color : nodeTagLockedColor,
                    background: nodeTagBg(isSel, lvl.color),
                    border: `1px solid ${nodeTagBorder(isSel, lvl.color)}`,
        ...
        {selLvl ? (
          <div className="flex items-center gap-3">
            ...
            <div className="w-14 h-14 shrink-0 rounded-2xl flex items-center justify-center text-3xl shadow-sm"
              style={{
                background: isDark ? selLvl.darkBg : selLvl.bg,
                border: `1.5px solid ${selLvl.color}40`,
              }}>
              {selLvl.emoji}
            </div>
            {/* Level info */}
          ...
          <p className="text-center text-sm py-2" style={{ color: emptyTextColor }}>{t.tapToStart}</p>
        )}
      </div>
